...
import { getHues, deleteHueById } from 'apiSdk/hues';
import { HueInterface } from 'interfaces/hue';

type ColumnType = ColumnDef<HueInterface, unknown>;

interface HueListPageProps {
...
export function HueListPage(props: HueListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<HueInterface>>(
    () => `/hues?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: HueInterface) => {
    if (hasAccess('hue', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/hues/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/hues/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('hue', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/hues/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('hue', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="hue" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Hues
        </Text>
        {hasAccess('hue', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/hues/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
